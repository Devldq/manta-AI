...
export class ToolRegistry {
  ...
  /**
   * 标记工具为"已发现"（供 tool_search 和自动发现机制使用）
   */
   ...
   *
   * 策略（方案 1 + 方案 3）：
   * - 内置工具：始终包含完整 Schema，无需 tool_search 发现即可直接调用
   * - MCP 工具：始终列出（避免 AI SDK 报 "unavailable tool"），但未发现的 MCP
   *   工具使用宽松 Schema 占位，首次调用时自动发现并转发到真实 execute
   * - tool_search：始终包含
   */
  toAISDKFormat(): Record<string, any> {
    ...
    return this.buildAISDKTools(allTools, discovered);
  }
  ...
  toAISDKFormatForAgent(
    ...
    return this.buildAISDKTools(agentTools, discovered);
  }
   ...
   * - 内置工具始终展示完整 Schema
   * - 未发现的 MCP 工具使用宽松占位 Schema，首次调用时自动发现
   * - 在 execute 外层包裹截断逻辑
   * - 根据 isConcurrencySafe 加全局读写锁
   *
   * @param toolList 要构建的工具列表
   * @param discovered 已发现的工具名集合
   */
  private buildAISDKTools(
    ...
    /** 未发现 MCP 工具的占位 Schema：接受任意参数，真实校验交给实际工具 */
    const PLACEHOLDER_SCHEMA = { type: 'object', additionalProperties: true };

    for (const tool of toolList) {
      const isBuiltin = !tool.mcpServer;
      const isDiscovered = discovered.has(tool.name) || tool.name === 'tool_search';
      const useFullSchema = isBuiltin || isDiscovered;
      ...
      result[tool.name] = {
        ...
        execute: async (input: any) => {
          ...
          if (!isBuiltin && !isDiscovered) {
            ...
            console.log(`[ToolRegistry] 自动发现 MCP 工具: ${tool.name}`);
          }
