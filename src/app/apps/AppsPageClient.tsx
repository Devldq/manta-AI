    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="relative" ref={menuRef}>
          <button
            className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-md hover:bg-border"
            onClick={(e) => {
              e.stopPropagation()
          ...
          {menuOpen && (
            <div
              className="absolute right-0 top-8 z-10 w-36 rounded-lg shadow-lg py-1"
              style={{
                background: 'var(--color-background)',
                border: '1px solid var(--color-border)',
              }}
            >
              ...
              />
              <MenuItem
                icon={app.status === 'archived' ? <Undo2 size={12} /> : <Archive size={12} />}
                label={app.status === 'archived' ? '恢复' : '归档'}
                onClick={(e) => {
                  e.stopPropagation()
...
// ─── 主页面组件 ───────────────────────────────────────────────
export default function AppsPageClient({ initialApps }: { initialApps?: AppConfig[] }) {
  ...
  // 如果有 initialApps，注入到 store
  useEffect(() => {
    if (initialApps && initialApps.length > 0 && apps.length === 0) {
