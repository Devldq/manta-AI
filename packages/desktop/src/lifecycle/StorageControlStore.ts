...
export class StorageControlStore {
  ...
  async setPhase(id:string,phase:string):Promise<void> { await this.mutate((catalog)=>{ const item=catalog.operations.find((value)=>value.id===id); if(!item) throw new Error(`Unknown operation ${id}`); item.phase=phase; item.updatedAt=new Date().toISOString() }) }
  async markRelaunching(id:string):Promise<void> { await this.mutate((catalog)=>{ const item=catalog.operations.find((value)=>value.id===id); if(!item) throw new Error(`Unknown operation ${id}`); item.status='running'; item.phase='relaunching'; item.updatedAt=new Date().toISOString() }) }
  async markSucceeded(id:string):Promise<void> { await this.mutate((catalog)=>{ const item=catalog.operations.find((value)=>value.id===id); if(!item) throw new Error(`Unknown operation ${id}`); item.status='succeeded'; item.phase='completed'; item.updatedAt=new Date().toISOString() }) }
  /** Migration I/O committed, but a new process has not yet proved the target healthy. */
  async completeOperation(id:string,backupRefs:BackupRef[],rollbackContext?:{previous:AshBootstrap;current:AshBootstrap}):Promise<void> { await this.mutate((catalog)=>{const item=catalog.operations.find((value)=>value.id===id);if(!item)throw new Error(`Unknown operation ${id}`);item.status='running';item.phase='committed';item.updatedAt=new Date().toISOString();item.backupRefs=BackupRefSchema.array().parse(backupRefs);item.rollbackContext=rollbackContext}) }
  ...
  /** Catalog and intent are committed under one lock so every crash point recovers safely. */
  async commitRelaunchIntent(intent:RelaunchIntent):Promise<void>{ const valid=RelaunchIntentSchema.parse(intent); await this.serialized(async()=>{const catalog=await this.readCatalog();const item=catalog.operations.find((value)=>value.id===valid.operationId);if(!item||item.status!=='running'||item.phase!=='committed')throw new Error(`Operation ${valid.operationId} is not committed`);item.phase='relaunching';item.updatedAt=new Date().toISOString();await writeJsonAtomic(this.catalogPath,catalog);await writeJsonAtomic(this.intentPath,valid)}) }
