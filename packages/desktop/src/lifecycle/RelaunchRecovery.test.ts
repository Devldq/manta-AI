...
describe('relaunch recovery', () => {
  it('restores only the exact operation backup and never selects a newer unrelated backup by mtime', async () => {
    ...
    await mkdir(exact.backupPath,{recursive:true}); await writeFile(join(exact.backupPath,'data.txt'),'exact'); await mkdir(join(a,'manta-ai-data','.ash-backups','newer-op','work'),{recursive:true}); await writeFile(join(a,'manta-ai-data','.ash-backups','newer-op','work','data.txt'),'wrong'); await mkdir(exact.targetPath,{recursive:true}); await writeFile(join(exact.targetPath,'data.txt'),'target')
