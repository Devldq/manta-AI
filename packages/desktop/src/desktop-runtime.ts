...
const controller = new DesktopLifecycleController({
  ...
  async openOnboarding() { disposeOnboarding?.(); onboardingHandoff=false; onboardingWindow = createOnboardingWindow(); disposeOnboarding = registerSecureOnboardingIpc({ ipcMain, getWindow: () => onboardingWindow, dialog, app, selections, bootstrapPath: bootstrapPath(), initializeStorage, completeInitialization: (onProgress) => controller.continueAfterOnboarding(onProgress), onInitialized() { const completedWindow=onboardingWindow; onboardingHandoff=true; setImmediate(() => { if (completedWindow && onboardingWindow===completedWindow) { disposeOnboarding?.(); disposeOnboarding=undefined; completedWindow.close() } }) }, onboardingUrl: onboardingPageUrl() }); const senderId = onboardingWindow.webContents.id; onboardingWindow.on('closed', () => { const completedHandoff=onboardingHandoff; onboardingHandoff=false; selections.clearSender(senderId); onboardingWindow = undefined; if (!quitting && !completedHandoff) app.quit() }) },
