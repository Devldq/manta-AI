...
const controller = new DesktopLifecycleController({
  ...
  async openOnboarding() { disposeOnboarding?.(); disposeOnboarding = registerSecureOnboardingIpc({ ipcMain, getWindow: () => onboardingWindow, dialog, app, selections, bootstrapPath: bootstrapPath(), initializeStorage, previewStorageParent }); onboardingWindow = createOnboardingWindow(); const senderId = onboardingWindow.webContents.id; onboardingWindow.on('closed', () => { selections.clearSender(senderId); onboardingWindow = undefined; if (!quitting) app.quit() }) },
