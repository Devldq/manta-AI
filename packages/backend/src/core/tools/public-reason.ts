...
const PUBLIC_REASON_SCHEMA = {
  ...
  description: [
    ...
    'For multiple independent read-only calls with one purpose, use the exact same shared rationale',
    'on every call and issue them together in one model step so the UI can present one batch.',
    'Put the rationale only in this field; do not also emit a separate narration line before the tool call.',
