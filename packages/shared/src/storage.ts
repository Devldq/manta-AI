...
export const StorageIpcRequestSchema = z.union([
  ...
  z.union([
    z.object({ channel: z.literal('storage:configure-git'), volumeId: z.string().min(1), mode: z.literal('local') }).strict(),
    z.object({ channel: z.literal('storage:configure-git'), volumeId: z.string().min(1), mode: z.literal('remote'), remoteUrl: GitRemoteUrlSchema, authRef: GitCredentialRefSchema.optional() }).strict(),
  ]),
