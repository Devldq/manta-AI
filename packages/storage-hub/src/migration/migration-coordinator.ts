...
export class MigrationCoordinator {
  ...
  async relocateVolume(volumeId: string, targetParentPath: string, operationId = randomUUID()): Promise<string> {
    return this.transaction(async () => {
      ...
      const sourceRoot = volumeRoot(volume); const finalRoot = volumeRoot(targetParentPath); await this.rejectOverlap(sourceRoot, finalRoot); await this.ensureAbsent(finalRoot)
      ...
      try {
        ...
        journal = { ...journal, phase: 'committing' }; const next = this.committedBootstrap(current, journal, { volumes: current.volumes.map((item) => item.id === volumeId ? { ...item, parentPath: targetParentPath, rootPath: undefined, updatedAt: new Date().toISOString() } : item) })
  ...
  async moveGroup(group: StorageGroupId, targetVolumeId: string, operationId = randomUUID()): Promise<string> {
    return this.transaction(async () => {
      ...
      const source = this.volume(current, sourceVolumeId); const target = this.volume(current, targetVolumeId); const sourcePath = join(volumeRoot(source), group); const targetPath = join(volumeRoot(target), group); await this.rejectOverlap(sourcePath, targetPath); await this.ensureAbsent(targetPath)
      const id = operationId; const staging = join(volumeRoot(target), '.ash-staging', id, group); const backup = join(volumeRoot(source), '.ash-backups', id, group); const inventory = await inventoryTree(sourcePath); await this.ensureCapacity(volumeRoot(target), inventory.bytes)
  ...
  async replaceGroupFromStaging(group: StorageGroupId, stagedGroupPath: string, operationId = randomUUID()): Promise<string> {
    return this.transaction(async () => {
      ...
      const target = join(volumeRoot(volume), group); const staging = join(volumeRoot(volume), '.ash-staging', operationId, group); const backup = join(volumeRoot(volume), '.ash-backups', operationId, group)
  ...
  async replaceGroupsFromStaging(groups: Array<{ group: StorageGroupId; source: string }>, operationId: string = randomUUID(), preflight?: () => Promise<void>): Promise<string> {
    ...
    return this.transaction(async () => {
      ...
      const targetFor = (group: StorageGroupId) => { const volume = this.volume(current, current.groupAssignments[group]); return join(volumeRoot(volume), group) }
  ...
  private async verifyTargets(value: AshBootstrap, journal: MigrationJournal, groups: StorageGroupId[]): Promise<void> { await this.options.store.write({ ...value, pendingMigration: { ...journal, phase: 'restarting' } }); await this.options.store.write({ ...value, pendingMigration: { ...journal, phase: 'verifying' } }); for (const group of groups) { const volume = this.volume(value, value.groupAssignments[group]); const root = join(volumeRoot(volume), group); await this.driver(group).reopen(root); const result = await this.driver(group).validate(root); if (!result.ok) throw new Error(result.error ?? `Post-commit validation failed for ${group}`) } }
  ...
  private async reopen(value: AshBootstrap, groups: StorageGroupId[]): Promise<void> { for (const group of groups) { const volume = this.volume(value, value.groupAssignments[group]); await this.driver(group).reopen(join(volumeRoot(volume), group)) } }
  ...
  private importPaths(value: AshBootstrap, journal: MigrationJournal, group: StorageGroupId): { target: string; staging: string; backup: string } {
    ...
    const volume = this.volume(value, value.groupAssignments[group]); const root = volumeRoot(volume); const target = join(root, group)
  ...
  private async writeManifests(value: AshBootstrap): Promise<void> { for (const volume of value.volumes) await this.writeManifest(volumeRoot(volume), volume, this.groupsFor(value, volume.id), value.generation, 'active') }
  ...
  private pathsFor(current: AshBootstrap, previous: AshBootstrap, journal: MigrationJournal): { source: string; final: string; staging: string; backup: string } { if (journal.kind === 'import') throw new Error('Import journal requires per-group paths'); if (journal.kind === 'volume') { const source = volumeRoot(this.volume(previous, journal.sourceVolumeId)); const final = volumeRoot(journal.targetParentPath!); return { source, final, staging: join(dirname(final), `${basename(final)}.migrating-${journal.id}`), backup: source } } const group = journal.groups[0]; const sourceVolume = this.volume(previous, journal.sourceVolumeId); const targetVolume = this.volume(current, journal.targetVolumeId!); const source = join(volumeRoot(sourceVolume), group); const final = join(volumeRoot(targetVolume), group); return { source, final, staging: join(volumeRoot(targetVolume), '.ash-staging', journal.id, group), backup: join(volumeRoot(sourceVolume), '.ash-backups', journal.id, group) } }
}
