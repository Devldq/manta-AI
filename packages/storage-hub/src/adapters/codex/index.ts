...
export class CodexAdapter implements AgentAdapter {
  ...
  async planImport(selection: AssetSelection, target: AgentInstallation): Promise<ImportPlan>
  async planImport(selectionOrTarget: AssetSelection | AgentInstallation, maybeTarget?: AgentInstallation): Promise<ImportPlan> {
    const target = maybeTarget ?? selectionOrTarget as AgentInstallation
    const selection = maybeTarget ? selectionOrTarget as AssetSelection : { schemaVersion: 1 as const, assetIds: (await this.inspect(target)).assets.map((asset) => asset.id) }
    const selected = new Set(selection.assetIds); const operations: PreviewFileOperation[] = []
    for (const skill of await discoverSkills(root(target, 'user-skills'))) if (selected.has(boundedId('codex-skill-', skill.name))) for (const file of skill.files) operations.push({ id: `read-skill-${hash(`${skill.name}:${file.relativePath}`).slice(0, 16)}`, kind: 'read', rootId: 'user-skills', nativePath: file.nativePath })
    if (selected.has('codex-instructions')) for (const instruction of await instructionFiles(root(target, 'codex-home'))) operations.push({ id: instruction.fileName === 'AGENTS.override.md' ? 'read-instructions-override' : 'read-instructions-base', kind: 'read', rootId: 'codex-home', nativePath: instruction.nativePath })
    const config = join(root(target, 'codex-home'), 'config.toml'); if ([...selected].some((id) => id.startsWith('codex-mcp-')) && await exists(config)) operations.push({ id: 'read-mcp-config', kind: 'read', rootId: 'codex-home', nativePath: config })
    return this.#plan('import', target, operations, selection) as ImportPlan
  }
  ...
  async apply(plan: ApprovedAdapterPlan): Promise<AdapterResult> {
    ...
    if (plan.kind === 'import') {
      const selectedIds = new Set(plan.selection.assetIds); const skillRoot = root(plan.target, 'user-skills'); const skillOperations = plan.operations.filter((operation) => operation.id.startsWith('read-skill-')); const selectedSkills = (await discoverSkills(skillRoot)).filter((skill) => selectedIds.has(boundedId('codex-skill-', skill.name))); const selectedSkillIds = new Set(selectedSkills.map((skill) => boundedId('codex-skill-', skill.name))); if ([...selectedIds].some((id) => id.startsWith('codex-skill-') && !selectedSkillIds.has(id))) throw new Error('Selected native Codex skill changed after approval'); const currentSkillPaths = selectedSkills.flatMap((skill) => skill.files.map((file) => file.nativePath)).sort((a, b) => a < b ? -1 : a > b ? 1 : 0); const approvedSkillPaths = skillOperations.map((operation) => operation.nativePath).sort((a, b) => a < b ? -1 : a > b ? 1 : 0); if (JSON.stringify(currentSkillPaths) !== JSON.stringify(approvedSkillPaths)) throw new Error('Selected native Codex skill file set changed after approval')
      ...
      const instructionOperations = plan.operations.filter((operation) => operation.id.startsWith('read-instructions-')); if (selectedIds.has('codex-instructions')) { const approvedInstructionPaths = instructionOperations.map((item) => item.nativePath); if (JSON.stringify(await instructionFilePaths(root(plan.target, 'codex-home'))) !== JSON.stringify(approvedInstructionPaths)) throw new Error('Selected native Codex instruction file set changed after approval'); const instructionSnapshots = []; for (const operation of instructionOperations) instructionSnapshots.push({ name: operation.id.endsWith('override') ? 'AGENTS.override.md' : 'AGENTS.md', bytes: await approvedRead(operation) }); const active = instructionSnapshots.find((item) => item.name === 'AGENTS.override.md' && item.bytes.toString().trim()) ?? instructionSnapshots.find((item) => item.name === 'AGENTS.md' && item.bytes.toString().trim()); if (active) await this.#assets.import({ schemaVersion: 1, id: 'codex-instructions', kind: 'instructions', name: active.name, files: [{ relativePath: active.name, bytes: active.bytes, sha256: hash(active.bytes) }] }) }
      const selectedMcpIds = new Set([...selectedIds].filter((id) => id.startsWith('codex-mcp-'))); const configOperation = plan.operations.find((operation) => operation.id === 'read-mcp-config'); if (selectedMcpIds.size && !configOperation) throw new Error('Selected native Codex MCP config changed after approval'); if (configOperation) { const servers = parseMcpServers((await approvedRead(configOperation)).toString()); const matched = new Set<string>(); for (const server of servers) { const id = boundedId('codex-mcp-', server.name); if (!selectedMcpIds.has(id)) continue; matched.add(id); const bindings = []; for (const literal of server.literals) { const secretReferenceId = await this.#secrets.storeLiteral({ value: literal.value, purpose: `codex-mcp:${server.name}:${literal.field}` }); secretReferenceIds.push(secretReferenceId); bindings.push({ field: literal.field, secretReferenceId }) } const metadata = projectionSafeMetadata(server.name, server.metadata, bindings); await this.#assets.import({ schemaVersion: 1, id, kind: 'mcp-server', name: server.name, metadata, ...(bindings.length ? { secretReferenceIds: bindings.map((item) => item.secretReferenceId) } : {}) }) } if (matched.size !== selectedMcpIds.size) throw new Error('Selected native Codex MCP server changed after approval') }
    } else {
  ...
  #plan(kind: 'import' | 'projection', target: AgentInstallation, operations: readonly PreviewFileOperation[], selection: AssetSelection = { schemaVersion: 1, assetIds: [] }): ImportPlan | ProjectionPlan { const createdAt = this.#now(); return { schemaVersion: 1, kind, planId: `codex-${kind}-${createdAt.getTime()}`, adapterId: this.id, target: structuredClone(target), selection: structuredClone(selection), operations, createdAt: createdAt.toISOString(), expiresAt: new Date(createdAt.getTime() + 60 * 60 * 1000).toISOString(), digest: '' } as ImportPlan | ProjectionPlan }
}
