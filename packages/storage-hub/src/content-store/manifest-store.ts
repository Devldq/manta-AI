...
export class AssetManifestStore {
  ...
  async remove(assetId: string, expected: { createdAt: string }): Promise<boolean> {
    ...
    let manifest: Required<AssetManifest>
    try { manifest = await this.read(assetId) } catch (error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return false; throw error }
    if (manifest.createdAt !== expected.createdAt) return false
