...
describe('StoragePathRouter', () => {
  it.each([['/parent', path.posix.join('/parent', '.manta-ai', 'work', 'a', 'b')], ['C:\\parent', path.win32.join('C:\\parent', '.manta-ai', 'work', 'a', 'b')]])('routes POSIX and Windows parents beneath the group root', (parent, expected) => expect(new StoragePathRouter(new VolumeRegistry(bootstrap(parent))).resolve('work', 'a', 'b')).toBe(expected))
