...
describe('Agent connections presentation', () => {
  ...
  it('shows every preview operation and verified-only savings, then exposes explicit apply and rollback', () => {
    const html = renderToStaticMarkup(<AgentConnectionsView connection={connection} assets={assets} reuse={{ scanStatus: 'complete', evidenceStatus: 'verified', portableAssetCount: 2, logicalImmutableBytes: 20, uniqueVerifiedObjectBytes: 12, verifiedSavedBytes: 8 }} selected={new Set(['portable-skill'])} preview={{ planSessionId: 'session-1', kind: 'projection', expiresAt: '2026-07-15T01:00:00.000Z', operations: [{ id: 'create', kind: 'create', rootId: 'user-skills', nativePath: '/home/.agents/skills/demo/SKILL.md', expectedAfterSha256: 'a'.repeat(64) }] }} result={{ operationId: 'operation-1', adapterId: 'codex', installationId: 'codex-user', kind: 'projection', phase: 'committed', status: 'committed', verified: true, completedAt: '2026-07-15T00:00:01.000Z', operationCount: 1 }} busy={false} onToggle={() => {}} onPreviewImport={() => {}} onPreviewProjection={() => {}} onApply={() => {}} onRollback={() => {}} />)
  ...
  it('renders structured progress and an error retry card while disabling mutation controls', () => {
    const html = renderToStaticMarkup(<AgentConnectionsView connection={connection} assets={assets} selected={new Set(['portable-skill'])} progress={{ operationId: 'operation-1', phase: 'applying', status: 'running', operationsCompleted: 1, operationsTotal: 3 }} error={new Error('Detection unavailable')} busy onToggle={() => {}} onPreviewImport={() => {}} onPreviewProjection={() => {}} onRetry={() => {}} />)
