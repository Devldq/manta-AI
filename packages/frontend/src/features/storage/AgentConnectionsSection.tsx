  ...
  return <section aria-labelledby="agent-connections-heading" style={{ marginTop: 24, borderTop: '1px solid var(--color-border)', paddingTop: 18 }}>
    ...
    {!adapter ? <p role="status">No supported Agent adapters.</p> : <article style={{ border: '1px solid var(--color-border)', borderRadius: 8, padding: 14 }}>
      ...
      {props.assets && <div style={{ display: 'grid', gap: 12 }}>
        <div><strong>Native assets available for import</strong>{GROUPS.map((group) => <fieldset key={`native-${group.kind}`} style={{ border: 0, padding: 0 }}><legend>{group.label}</legend>{props.assets!.inventory.assets.filter((item) => item.kind === group.kind).map((item) => <label key={item.id} style={{ display: 'block' }}><input type="checkbox" checked={props.nativeSelected.has(item.id)} disabled={props.busy} onChange={() => props.onToggleNative(item.id)} /> <code>{item.nativePath}</code></label>)}</fieldset>)}</div>
        <div><strong>Portable ASH assets for projection</strong>{GROUPS.map((group) => <fieldset key={`portable-${group.kind}`} style={{ border: 0, padding: 0 }}><legend>{group.label}</legend>{props.assets!.portableAssets.filter((item) => item.kind === group.kind).map((item) => <label key={item.id} style={{ display: 'block' }}><input type="checkbox" checked={props.portableSelected.has(item.id)} disabled={props.busy} onChange={() => props.onTogglePortable(item.id)} /> {item.id}</label>)}</fieldset>)}</div>
        <div><button disabled={props.busy || props.nativeSelected.size === 0} onClick={props.onPreviewImport}>Preview import</button> <button disabled={props.busy || props.portableSelected.size === 0} onClick={props.onPreviewProjection}>Preview projection</button></div>
      </div>}
...
export function AgentConnectionsSection() {
  const [connection, setConnection] = useState<AgentConnectionState>({ adapters: [{ id: 'codex', displayName: 'Codex', status: 'not-detected', installations: [] }], operations: [] }); const [assets, setAssets] = useState<AgentAssets>(); const [reuse, setReuse] = useState<AgentReuseMetrics>(); const [nativeSelected, setNativeSelected] = useState<Set<string>>(new Set()); const [portableSelected, setPortableSelected] = useState<Set<string>>(new Set()); const [preview, setPreview] = useState<AgentPlanPreview>(); const [progress, setProgress] = useState<AgentStorageProgress>(); const [result, setResult] = useState<AgentOperationSummary>(); const [error, setError] = useState<Error>(); const [busy, setBusy] = useState(false); const gate = useMemo(() => createSubmissionGate(setBusy), [])
  const refresh = useCallback(async () => { try { const state = await storageApi.agents(); const installation = state.adapters[0]?.installations[0]; const loaded = installation ? await Promise.all([storageApi.agentAssets(state.adapters[0]!.id, installation.id), storageApi.agentReuse()]) : undefined; setConnection(state); setAssets(loaded?.[0]); setReuse(loaded?.[1]); setNativeSelected(new Set()); setPortableSelected(new Set()); setError(undefined) } catch (reason) { setError(reason as Error) } }, [])
  useEffect(() => { void refresh(); return desktopStorageBridge()?.subscribeAgentProgress?.(setProgress) }, [refresh])
  const run = (operation: () => Promise<void>) => gate.run(async () => { try { await operation(); setError(undefined) } catch (reason) { setError(reason as Error) } })
  const adapter = connection.adapters[0]; const installation = adapter?.installations[0]
  const plan = (kind: 'import' | 'projection') => run(async () => { if (!adapter || !installation) return; setProgress(undefined); const response = await invokeStorage(kind === 'import' ? { channel: 'storage:agent-plan-import', adapterId: adapter.id, installationId: installation.id, assetIds: [...nativeSelected] } : { channel: 'storage:agent-plan-projection', adapterId: adapter.id, installationId: installation.id, assetIds: [...portableSelected] }); if (response.kind !== 'agent-plan') throw new Error('Agent preview was unavailable'); setPreview(response.plan); setResult(undefined) })
