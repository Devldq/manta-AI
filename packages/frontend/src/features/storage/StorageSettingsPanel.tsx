...
export function StorageSettingsPanel() {
  ...
  const configureGit = useCallback(async (request: Extract<Parameters<typeof invokeStorage>[0], { channel: 'storage:configure-git' }>) => { const result = await invokeStorage(request); if (result.kind !== 'git-configured') throw new Error('Git configuration did not return a binding'); await refresh() }, [refresh])
  ...
  const setGitSecretsPolicy = useCallback(async (volumeId: string, includeSecrets: boolean, grant?: string) => { const request = includeSecrets ? { channel: 'storage:set-git-secrets-policy' as const, volumeId, includeSecrets: true as const, grant: grant! } : { channel: 'storage:set-git-secrets-policy' as const, volumeId, includeSecrets: false as const }; const result = await invokeStorage(request); if (result.kind !== 'git-secrets-policy') throw new Error('Git secrets policy was not updated'); await refresh() }, [refresh])
  const syncGit = useCallback(async (volumeId: string) => { const result = await invokeStorage({ channel: 'storage:sync-volume', volumeId }); if (result.kind !== 'completed') throw new Error('Git sync did not complete'); await refresh() }, [refresh])
  ...
  const applyGitImport = useCallback(async (volumeId: string, plan: StorageGitImportPlan, decisions: Extract<Parameters<typeof invokeStorage>[0], { channel: 'storage:apply-git-import' }>['decisions']) => { const result = await invokeStorage({ channel: 'storage:apply-git-import', volumeId, sessionId: plan.sessionId, decisions }); if (result.kind !== 'completed') throw new Error('Git import did not complete'); await refresh() }, [refresh])
  const createVolume = () => setDialog({ title: 'Create storage volume', body: `Choose a parent folder. ASH will create a ${ASH_VOLUME_DIR_NAME} directory inside it.`, action: async () => { const selected = await invokeStorage({ channel: 'storage:select-parent', purpose: 'createVolume' }); if (selected.kind !== 'parent-selected' || !selected.selectionId) return; const created = await invokeStorage({ channel: 'storage:create-volume', selectionId: selected.selectionId, name: `Volume ${volumes.length + 1}` }); if (created.kind !== 'volume-created') throw new Error('Storage volume creation did not return a volume identifier'); await refresh() } })
  ...
  }, [applyGitImport, backups, busy, configureGit, error, gitBindings, gitCapability, loading, overview, planGitImport, refresh, requestGitSecretsGrant, setGitSecretsPolicy, syncGit, volumes])
