...
export function StorageSettingsPanel() {
  ...
  const content = useMemo(() => {
    ...
    return <>
      ...
      <h3 style={{ marginTop: 20 }}>Volumes</h3><div style={{ display: 'grid', gap: 10 }}>{volumes.map((volume) => <StorageVolumeCard key={volume.id} volume={volume} bytes={volume.inventory?.bytes} files={volume.inventory?.files} disabled={busy} git={{ ...gitCapability, binding: gitBindings.find((binding) => binding.volumeId === volume.id) }} onConfigureGit={configureGit} onSync={() => syncGit(volume.id)} onPlanImport={() => planGitImport(volume.id)} onApplyImport={(plan, decisions) => applyGitImport(volume.id, plan, decisions)} onOpen={() => void run({ channel: 'storage:open-volume', volumeId: volume.id })} onRelocate={() => migrateVolume(volume)} />)}</div>
