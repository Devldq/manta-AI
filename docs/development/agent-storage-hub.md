...
`@manta/storage-hub` is the Node/TypeScript core for Manta AI volume registry, routing, inventory, migration, Git/cloud synchronization, and content-addressed storage. Desktop owns privileged lifecycle and IPC; Backend receives routed group roots; Frontend uses typed HTTP/IPC contracts. Production persistence must not construct its own home-directory data root. External Agent import/projection adapters are intentionally out of scope.
...
Phase scripts are the durable acceptance entry points. Phase 1 covers storage, desktop, browser migration, recovery, and audit evidence. Phases 2–3 cover Manta AI Git/cloud synchronization and truthful volume-local content accounting.
...
For a visual release check, use an isolated user-data profile and verify the required onboarding screen, `manta-ai-data` wording, disabled confirmation before selection, successful main-window launch, Storage volume/group metrics, migration controls, and Git/cloud status. Do not commit `release`, `.package-staging`, coverage, Turbo logs, user-data profiles, `manta-ai-data` volumes, credentials, or smoke markers.
